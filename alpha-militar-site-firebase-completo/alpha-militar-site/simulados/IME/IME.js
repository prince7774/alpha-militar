...
/* SIMULADOS IME COMPLETOS - Alpha Militar
...
const simulados = {
  "Simulado Diagnóstico": {
    ...
    descricao: "50 questões para medir sua base no padrão IME. Mistura Matemática, Português, História, Geografia, Inglês e Redação.",
    tempoMinutos: 150,
    questoes: [
      ...
      {materia:"Português", dificuldade:"Médio", pergunta:"Na frase 'Os alunos da IME chegaram cedo', o núcleo do sujeito é:", opcoes:["ESA","cedo","alunos","chegaram","da IME"], correta:"alunos", explicacao:"O núcleo do sujeito é o substantivo principal 'alunos'."},
...
simulados["Simulado IME 01"].questoes = bancoESA01.slice(0, 50);
simulados["Simulado IME 02"].questoes = bancoESA02.slice(0, 50);
...
function abrirSimulado(nome){
  ...
  box.innerHTML = `
    <div class="simulado-info-box">
      ...
      <span>${simuladoAtual.questoes.length} questões · nível IME · correção automática</span><br>
...
function corrigirSimulado(tempoEsgotado = false){
  ...
  const desempenho = nota >= 80 ? 'Excelente. Você já está competitivo no padrão IME.' : nota >= 65 ? 'Bom. Falta lapidar os erros e ganhar velocidade.' : nota >= 50 ? 'Regular. Reforce teoria e refaça as questões erradas.' : 'Atenção. Volte às bases e faça revisão por matéria.';
