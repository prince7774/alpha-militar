...
/* SIMULADOS ITA COMPLETOS - Alpha Militar
...
const simulados = {
  "Simulado Diagnóstico": {
    ...
    descricao: "50 questões para medir sua base no padrão ITA. Mistura Matemática, Português, História, Geografia, Inglês e Redação.",
    tempoMinutos: 150,
    questoes: [
      ...
      {materia:"Português", dificuldade:"Médio", pergunta:"Na frase 'Os alunos da ITA chegaram cedo', o núcleo do sujeito é:", opcoes:["ESA","cedo","alunos","chegaram","da ITA"], correta:"alunos", explicacao:"O núcleo do sujeito é o substantivo principal 'alunos'."},
...
simulados["Simulado ITA 01"].questoes = bancoESA01.slice(0, 50);
simulados["Simulado ITA 02"].questoes = bancoESA02.slice(0, 50);
...
function abrirSimulado(nome){
  ...
  box.innerHTML = `
    <div class="simulado-info-box">
      ...
      <span>${simuladoAtual.questoes.length} questões · nível ITA · correção automática</span><br>
...
function corrigirSimulado(tempoEsgotado = false){
  ...
  const desempenho = nota >= 80 ? 'Excelente. Você já está competitivo no padrão ITA.' : nota >= 65 ? 'Bom. Falta lapidar os erros e ganhar velocidade.' : nota >= 50 ? 'Regular. Reforce teoria e refaça as questões erradas.' : 'Atenção. Volte às bases e faça revisão por matéria.';
